...
#
# 🔴 閘門回 2（掃描器自己讀不懂某個檔）走 **EQUIP**，不進 FAIL，也**不計入 TOTAL** ——
#    它沒有產生判定。本腳本的退出碼因此是三態（檔尾那行：FAIL 優先於 EQUIP），
#    而 security_regression_daily.sh 的 classify() 有對應的 `instrument` 態，
#    通知會說「這不是安全回歸」。
#
# ⚠️ **這段註解 2026-09-12 訂正過，原文留著當例子**：它原本寫
#    「本腳本只有兩態、表達不出第三態，要分三態得先改 classify()，那是另一件事」。
#    我當天稍後就把三態做完了，**卻只更新了設計冊那個載體，漏掉這裡** ——
#    而這裡才是維護這段程式的人真正會讀到的那一份（Codex 覆驗 P2 抓到）。
#    ⇒ 機制升級時，回頭 grep 描述**舊行為**的關鍵詞（那次是「兩態」），
#    而不是只改我當下正在編輯的那個檔。
