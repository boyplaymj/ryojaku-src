...
##### 🔴 而豁免通道自己被誤開過（2026-09-12 Codex 覆驗 P2）

我**宣告**的格式是 `# RC2-EXEMPT: <理由>`，**實作**卻是 `if EXEMPT_MARK in ln`
—— 任意一行的 substring。⇒ docstring 裡提到它、字串常數裡寫它、理由留空，**全都放行**。
**宣告與實作不符，而不符的方向是「後門被誤開」** —— 那比漏擋更糟，
因為輸出看起來完全正常（清單上就是一行 🟡 豁免）。

修法是用 **`tokenize` 的 COMMENT token**，不自己判「這行是不是註解」——
後者要處理字串裡的 `#`、行內註解、三引號……**那又是一份手寫規則**。

| | 檔案內容 | 讀數 |
|---|---|---|
| E1 | 標記寫在 **docstring** | **rc=1**，並印「不在註解裡」 |
| **E2** | `s = "# RC2-EXEMPT: 假的"`（**字串常數偽裝成註解**） | **rc=1** ← 只有 tokenize 分得出來 |
| E3 | 真註解但**理由留空** | **rc=1**，並印「理由是空的」 |
| E4 | 真註解 ＋ 理由 | **rc=0**，🟡 豁免並印出理由 |
| E5 | **行內**註解 ＋ 理由 | **rc=0**（行內也是註解，刻意接受） |

🔴 **E1/E2/E3 一定要印出「豁免為什麼不成立」** —— 否則「標記寫了但不算數」與
「我根本沒寫標記」在輸出上逐字相同，作者會以為這個機制壞了。
...
~~🔴 **一個我消不掉的取捨**：`security_regression.sh` 只有兩態……
要真的分三態得先改 `classify()`，那是另一件事，沒做。~~
✅ **2026-09-12 已做完，見下一節。**
...
### ✅ 每日守衛真的分三態了 —— 儀器問題不再被通知成安全回歸（2026-09-12）

上一節結尾那個「消不掉的取捨」收掉了。兩層都改：

**內層 `security_regression.sh`**：新增與 `FAIL` **分開**的 `EQUIP` 計數器。
G-5 閘門回 2 時記 `EQUIP`，**且不計入 `TOTAL`** —— 計入的話「通過 N/N」會把
一格沒有判定的東西算成通過了。摘要行變成
`══ 斷言：通過 X / 共 Y（失敗 Z，儀器 W）══`，退出碼三態。
...
🔴 **`FAIL` 優先於 `EQUIP`**，理由要講清楚：真的量到的回歸是更急的訊號，
而且它**有判定**；儀器問題只影響它自己那一格，不使其餘上百條斷言失效。
⚠️ **這與 `verify_admin_role_gate.py` 的規則相反**（那裡 equip 優先）——
因為在那支裡「沒量到」代表**整個維度**都沒跑，剩下的綠燈撐不起結論。
**判準是「沒量到的範圍有多大」，不是「哪個碼比較大」。**

**外層 `classify()`**：新增 `instrument` 態，排在 `assert` 之後、**`cleanup` 之前**
（內層回 rc=2，而 `rc!=0` 會被 `cleanup` 那條先接走 —— 排錯順序等於這道分流白做）。
通知文案分開：🟠「儀器沒跑成」並明說「**這不是安全回歸**，要查的是那支工具」，
同時提醒「**也不要讀成通過**：那幾格今天沒有判定」。

**驗收**：`--selftest` **15 條**（由 11 條加到 15，新增四條全是這一格），
其中三條是反控 —— ①儀器＋rc=1 仍判 `instrument`（釘住排序）②斷言紅＋儀器 1 判 `assert`
（釘住優先權）③**舊格式 log 沒有「儀器」欄仍判 `green`**（向後相容：抓到空字串要當 0）。
突變 `infra/classify_mutate.py` **3 發全殺**（排序對調／不解析該欄／優先權反過來）。

#### ⚠️ 寫突變腳本時踩的兩層跳脫

第一版把突變寫在**巢狀 heredoc** 裡，`$` 與 `{` 被 shell 吃掉 ⇒ 兩發 `ValueError`
（看得出來），**一發靜靜沒改到任何東西** —— 那是「no-op 突變體 ≡ 測試有洞」，
報表上與「測試沒牙」逐字相同。
⇒ 突變改寫成獨立 `.py`，而且**每一發都斷言「檔案真的變了」**；
正則錨點一律用 **raw string**（跳脫層數我錯了兩次：shell heredoc 一層、Python 字面值一層）。

⚠️ **界線**：整支 `security_regression.sh` 沒有端到端重跑過 —— 它每次註冊 2 個帳號，
而 `app-register` 限流是每 IP 每小時 10 次。本輪驗的是**抽出來的判定邏輯**
（三態退出碼 ＋ 摘要行解析）與 `classify()` 的 selftest／突變。
**第一次真正的端到端讀數要等明天 02:15 的排程。**

### 🔴 同一個事實兩個載體，我更新的是「當下在編輯的那個」（2026-09-12）

Codex 覆驗的 P2：`security_regression.sh` 的 G-5 節**註解**還寫著
「本腳本只有兩態、表達不出第三態，要分三態得先改 `classify()`，**那是另一件事**」——
而它就**印在實作三態的那段程式正上方**（`*)` 分支記的是 `EQUIP`，檔尾是三態退出碼）。

🔴 **失效的方向值得記**：我在做三態那一顆 commit 裡，**有**把設計冊那句劃掉並指向新節，
**卻沒改程式裡這句**。而程式裡這句才是**維護這段程式的人真正會讀到的那一份** ——
設計冊要人主動去翻，註解是打開檔案就撞見。
⇒ 這是 `reference_fixed_the_occurrence_i_happened_to_read` 的第 N 次：
**我更新的是「當下正在編輯的那個載體」，不是「最靠近讀者的那個」。**

⇒ 判別法：**機制升級時，回頭 `grep` 描述舊行為的關鍵詞**（這次是「兩態」），
而不是只改手上那個檔。實際掃完全 repo 只有兩處，另一處已劃掉。
順手也掃了本輪碰過的七個檔的「尚未／那是另一件事／TODO」——
只命中 `security_regression_daily.sh:18` 那句，而它講的是**仍然成立**的界線
（看不見未提交的改動），不是殘留。

⚠️ **這一格沒有機制。** 「註解與程式相矛盾」一般而言不可判定；
我沒有替它加閘，因為任何窄到能實作的版本都會變成另一份手寫關鍵詞清單
（本輪已經因為手寫清單被咬過三次）。**它靠的是上面那個習慣，不是 exit code。**

### ✅ `admin-users` 的分頁分支量到了 —— 而我兩次估份量都貴了一個數量級（2026-09-12）

`infra/verify_users_paging_live.py`，**6 格全過**（rc=0）。

#### 🔴 先講我錯在哪：那句「要種到 scan 超過 1 MB／幾百筆」是憑空估的

| 我說過的 | 實際 |
|---|---|
| 「要 scan 超過 **1 MB** 才有 `LastEvaluatedKey`」 | handler 設了 `Limit` ⇒ 門檻是**筆數**，不是位元組 |
| 「要種**幾百筆**再刪」 | `limit := 20` **寫死在程式裡**（不吃 query param）⇒ 總數 >20 就有游標。現況 6 筆 ⇒ **種 15 筆** |

⇒ 兩次都是**沒讀那段程式就估份量**，而估出來的份量貴了一個數量級 ——
**那會讓一件做得到的事看起來不值得做**，於是它在待辦上躺了好幾輪。
（同一個形狀：`reference_offered_as_option_what_git_says_is_done` 的親戚 ——
選項清單的份量欄也是一種宣稱。）
...
| | 做什麼 | 讀數 |
|---|---|---|
| **A** | 種之前（6 筆 ≤ 20） | `lastKey == ""` ← **基準**，少了它「非空」可能是常數 |
| **B** | 種 15 筆（共 21）後 | `lastKey` **非空** ⇒ `if result.LastEvaluatedKey != nil` 那條真的走到了 |
| B' | 第一頁筆數 | **20**（`limit` 生效） |
| **C** | 把 `lastKey` 當 `?lastKey=` 傳回去 | 第二頁與第一頁**交集 0** ⇒ 它是真游標，不是「一個非空字串」 |
| C' | 第二頁 | 非空 |
| **D** | 清理後 | 殘留 **0**、`lastKey` 回到 `""`、表回到 **6 筆** |

🔴 **C 那格是必要的**：只有 B 的話，「回了一個非空字串」與「回了正確的游標」
在讀數上分不出來 —— 而那正是這段程式唯一會出錯的地方。

#### 打不到的那一半

`else`（`LastEvaluatedKey["userId"]` 不是字串）**結構上不可構造**：
表的 key schema 是 `userId` HASH、型別 `S` ⇒ DynamoDB 回的永遠是 `AttributeValueMemberS`。
**本支不宣稱驗過它**（同那 6 支 `validateToken` 的 `!ok`）。
它的價值仍是預防：將來有人改 key schema 或加 range key 時不會變成 panic。

#### ⚠️ 探針第一版自己壞了，而 rc=2 擋住了

我照慣例猜回應是 `{data:{users:[{userId}]}}`，實際是
**頂層** `{data:[...], lastKey, meta, success}` 且 user 物件的鍵是 **`id`** 不是 `userId`
⇒ `AttributeError` ⇒ **rc=2「探針自己壞了，不可讀成通過」**。
那正是這一輪建立的三態約定第一次在真實情境下擋住我 ——
舊寫法（例外冒出去 ⇒ rc=1）會讓它看起來像「被測物壞了」。

⚠️ **副作用**：對**共用的 stg `Users` 表**寫入 15 筆（前綴 `probe-paging-`），跑完刪除並讀回。
那張表 `security_regression.sh` 也在用 —— 它的清理只認自己那次的 MARK/HOST/GID，
不會碰我的；反之我只刪自己前綴的。⚠️ 時間上也錯開（每日排程 02:15 UTC，本次 07:5x）。
