...
//
// 釘的是 2026-09-03 的決定：(isOwner || joined) && status !== 'cancelled'，
// **不綁** completed（那是評價的條件，判台正好相反）。要改條件先來改這裡。
...
test('A1b-5 recruiting／full／closed 都看得到 —— 釘住「不綁 completed」', () => {
    for (const status of ['recruiting', 'full', 'closed']) {
