...
test('【正控】而且它真的有在顯示台數（不是連 totalTai 一起被拿掉）', () => {
  ...
  //
  // ⚠️ 2026-09-04（D5-d）放寬：原本寫死 `totalTai(TABLE, sel)`。
  //    D5-d 讓這一頁的表可以在執行期換成後台下發的那份，模組層常數因此改名
  //    BUNDLE、傳給 totalTai 的變成區域變數 `table` ⇒ 這條當場轉紅。
  //    它紅得對（原始碼真的變了），但**紅的理由是識別字改名，不是台數不顯示了** ——
  //    把變數名寫進守衛，等於讓每一次無關的更名都變成假警報，
  //    而假警報訓練出來的忽略不可逆。⇒ 只釘「有沒有拿 totalTai 去算 sel」。
  assert.match(PAGE, /totalTai\(\s*\w+\s*,\s*sel\s*\)/, '報台頁應該用 totalTai 算要顯示的台數');
});
