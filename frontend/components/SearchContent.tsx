// components/SearchContent.tsx — 「找場次」的內容（[A2-a-2] 從 pages/Search.tsx 原樣搬過來）
//
// 為什麼搬：揪咖頁（pages/Matchmaking.tsx）的「找場次」tab 要用同一份，而 /search 路由
// 在 [A2-b] 動底部導覽之前還得活著。直接把 <SearchPage /> 塞進 tab 會讓兩條 sticky 列
// （揪咖的 tab 列＋這裡的「探索所有／附近場次」）貼在同一個 top、後者蓋掉前者 ⇒
// 抽成元件、由殼傳 stickyTopClass。除了那一個 class 與 import 路徑，邏輯一行沒改。
//
...
/**
 * 「探索所有／附近場次」那條子 tab 列是 sticky 的；它貼在哪個高度取決於上面有什麼。
 * 單獨當 /search 頁時上面只有 TopBar（4rem）；塞進揪咖頁時上面還多一列 tab（見 Matchmaking.tsx）。
 * 做法同 components/ledger/LedgerContent.tsx 的 monthBarStickyTopClass：由殼告訴內容。
 */
export const SEARCH_STICKY_TOP_DEFAULT = 'top-[calc(4rem+env(safe-area-inset-top))]';

export interface SearchContentProps {
    /** 子 tab 列 sticky 的 top class。預設＝只有 TopBar 在上面。 */
    stickyTopClass?: string;
}

const SearchContent: React.FC<SearchContentProps> = ({ stickyTopClass = SEARCH_STICKY_TOP_DEFAULT }) => {
    ...
    return (
        <div className="pb-4 flex flex-col w-full">
            ...
            <div className={`h-16 px-4 flex items-center bg-white border-b border-black/[0.03] sticky ${stickyTopClass} z-20`}>
