// components/ledger/LedgerContent.tsx — 帳本的內容元件（[A1-a-4]）
//
// pages/Ledger.tsx 原本用一個不透明布林同時決定四件事（要不要自動開彈窗、容器頂部留白、
// 畫不畫 header、月份列 sticky 的 top），這裡把它們拆成講得出名字的 props（見 LedgerContentProps）。
// state 與 handler 都在這裡：兩條到達路徑（路由 /ledger、Profile → LedgerOverlay）各自 mount 一份，不共享。
//
// ⚠️ 這個 repo 沒有元件測試 —— typecheck／build 綠不代表兩條路徑的畫面沒壞。
...
import LedgerShareCard from '../ledger/LedgerShareCard';
import LedgerEntryFormModal from '../ledger/LedgerEntryFormModal';

import { AppSelect } from '../ui/CommonUI';
...
export interface LedgerContentHeaderActions {
    /** 清空表單、退出編輯模式、打開「建立新紀錄」彈窗（原本頁面 header 那顆 ＋ 的行為） */
    openNewEntry: () => void;
}

export interface LedgerContentProps {
    /** 帶 ?gameId= 進來時要不要自動打開新增彈窗。獨立頁 /ledger 要；Profile → overlay 那條不要（避免從個人頁誤開）。 */
    autoOpenAddModalOnGameId: boolean;
    /** 月份選擇列 sticky 的 top class。有頁面 header 時要讓出 4rem＋safe-area；overlay 內容在自己的捲動容器裡，給 top-0。 */
    monthBarStickyTopClass: string;
    /** 最外層容器額外的頂部留白 class（overlay 用 pt-8；獨立頁不給）。 */
    topPaddingClass?: string;
    /** 頁面殼的 header。要畫在這個元件的容器**裡面**（它靠 -mx-5 px-5 對齊容器、且 loading 時要跟著整頁一起不畫），所以用 render prop 而不是外面包一層。 */
    renderHeader?: (actions: LedgerContentHeaderActions) => React.ReactNode;
    /** overlay 那條「按了＋」的土砲事件：數字遞增一次＝打開一次彈窗（不清空表單）。照 LedgerOverlay 原樣接，不要改成別的模式。 */
    onAddActionTrigger?: number;
    /** 導去語音判台之前呼叫（overlay 傳 onClose）。見下方語音判台入口那段註解：留著無害，但不是必要的。 */
    onBeforeNavigateAway?: () => void;
}

const LedgerContent: React.FC<LedgerContentProps> = ({
    autoOpenAddModalOnGameId,
    monthBarStickyTopClass,
    topPaddingClass = '',
    renderHeader,
    onAddActionTrigger,
    ...
    useEffect(() => {
        ...
        // 只有獨立頁 /ledger 會自動打開新增彈窗（autoOpenAddModalOnGameId）；overlay 由 Profile 開，不要
        if (gameId) {
    ...
    }, [fetchData, gameId, autoOpenAddModalOnGameId]);
    ...
    // 頁面 header 那顆 ＋ 的行為（原本寫在 header 的 onClick 裡）：清空表單、退出編輯、開彈窗。
    // 跟 onAddActionTrigger 那條不同：那條只 setShowAddModal(true)、不清空，是 overlay 既有行為，兩者都照舊。
    const openNewEntry = () => {
        setIsEditing(false);
        setFormData({
            date: new Date().toISOString().split('T')[0],
    ...
    return (
        <div className={`px-5 max-w-7xl mx-auto w-full animate-fade-in bg-[#f9f9f7] min-h-screen ${topPaddingClass}`}>
            {/* Top Header - 由頁面殼（pages/Ledger.tsx）透過 renderHeader 提供；overlay 路徑沒有 */}
            {renderHeader?.({ openNewEntry })}

            {/* Month Selector - Redesigned Capsule Style */}
            <div className={`flex mb-3 sticky ${monthBarStickyTopClass} z-[40] bg-[#f9f9f7]/95 backdrop-blur-md -mx-5 px-5 py-2 transition-all border-b border-black/[0.01]`}>
            ...
            {/* 語音判台入口（訓練工具）—— 只導頁，不接記帳資料。
                🔴 放在這裡而不是 header：header 是頁面殼（pages/Ledger.tsx）透過 renderHeader 畫的，
                Profile → LedgerOverlay 那條路徑沒有這個 header（overlay 自己那個只有返回＋新增），
                入口放 header 會在那條路徑上消失。
                正典 tools/mahjong-tai/DESIGN_APP.md §6／§11 待辦「入口放哪」。 */}
            <button
                onClick={() => {
                    // overlay 那條路徑會把 onClose 接進來。它留著無害但不是必要的：
                    // LedgerOverlay 的生命週期屬於 Profile，換路由時 Profile 整個卸載、portal 跟著消失，
                    // 不呼叫也不會蓋在判台頁上面（DESIGN_APP.md §6a-2 突變 M2 量過）。
                    onBeforeNavigateAway?.();
